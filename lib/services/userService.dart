...
class UserService {
  ...
  // }
    ...
    Future<List<User>>getAllUsers()async{
      ...
      if(res.statusCode == 200){
         ...
         AlertDialog(content: AppText(res.statusMessage),) ;
         return null;
      }
  }
 ...
 Future<User> addNewUser(User user) async {
  ...
  Future<User> getUserByNameAndPassword(User user) async{
          Dio dio = Dio();
      Response res = await dio.get(API_URL+"/users?username=${user.name}&password=${user.password}");
      if(res.statusCode == 200){
          var users = res.data as List<dynamic>;
          if(users.length == 1){
          User matchedUser =User.fromJson(users.firstWhere((user) => user != null));
                return matchedUser;
          }else return null;
  ...
  Future<User>updateUserToken(User user,String token)async{
      Dio dio = Dio();
   dio.options.headers['content-Type'] = 'application/json';
      Response res = await dio.patch(API_URL+"/users/${user.id}",data:{"token":user.token});
      if(res.statusCode == 200){
          return User.fromJson(res.data);
      }else {
          AlertDialog(content: AppText(res.statusMessage),) ;
  ...
  Future<String> isUserSignedIn()async {
    SharedPreferences prefs = await LocalizationService().getSharedPreferences();
...
//5- if user get user by token  // 2- in any page if user logged use this method
  ...
  Future<User>addToCart(User user,num productId)async{
      ...
      Dio dio = Dio();
   dio.options.headers['content-Type'] = 'application/json';
      Response res = await dio.patch(API_URL+"/users/${user.id}",data:{"cart":cart});
      if(res.statusCode == 200){
          return User.fromJson(res.data);
      }else {
          AlertDialog(content: AppText(res.statusMessage),) ;
  ...
  // in log out remove user token 
    Future<User>deleteTokenByUserId(User user)async{
      ...
      Dio dio = Dio();
   dio.options.headers['content-Type'] = 'application/json';
      Response res = await dio.patch(API_URL+"/users/${user.id}",data:{"token":null});
      if(res.statusCode == 200){
          return User.fromJson(res.data);
      }else {
          AlertDialog(content: AppText(res.statusMessage),) ;
